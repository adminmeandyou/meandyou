...
export async function POST(req: NextRequest) {
  try {
    ...
    } catch (err) { console.error('[moderar-foto] analytics_events indisponível — ignorando rate limit:', err) }
    ...
    if (sightengineUser && sightengineSecret) {
      ...
      sightengineForm.append('api_user', sightengineUser)
      sightengineForm.append('api_secret', sightengineSecret)
    ...
    void supabaseAdmin.rpc('award_xp', { p_user_id: user.id, p_event_type: 'photo_added', p_base_xp: 10 }).then(() => {})
