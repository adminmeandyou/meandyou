...
// ✅ CORREÇÃO: usar Link do next/link em vez de <a> para navegação interna
...
export default function BuscaPage() {
  ...
  // Sincroniza filtros locais ao abrir o painel
  useEffect(() => {
    setLocalFilters(filters)
  }, [filters, showFilters])

  // Busca inicial com localização
  useEffect(() => {
    handleInitialSearch()
  }, [])
  ...
  async function handleInitialSearch() {
    ...
    // ✅ CORREÇÃO: chama search sem argumento — usa o estado de filtros atual
    // updateLocation já atualiza as coords no hook antes de search ser chamado
    search()
  }

  // ✅ CORREÇÃO: aplicar filtros primeiro via setFilters, depois disparar search via useEffect
  // para evitar race condition onde search() roda com filtros antigos
  ...
  // Dispara search quando filters de fato mudam no estado do hook
  useEffect(() => {
    if (pendingSearch) {
      search()
      setPendingSearch(false)
    }
  }, [filters, pendingSearch])
      ...
      <main className="px-4 pb-24">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 gap-3 text-white/30">
            ...
            <button onClick={handleReset} className="mt-2 text-[#b8f542] text-xs underline">
              Resetar filtros
            </button>
          </div>
        ) : (
          ...
          <div className="relative w-full bg-[#141020] rounded-t-3xl border-t border-white/10 p-6 z-10 max-h-[85vh] overflow-y-auto">
            ...
            <FilterSection title="Distância máxima">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-[#b8f542] font-semibold text-sm">{localFilters.maxDistanceKm} km</span>
              </div>
            ...
            <FilterSection title="Faixa de idade">
              <div className="flex gap-3">
                <div className="flex-1">
                  ...
                  <select value={localFilters.minAge}
                ...
                <div className="flex-1">
                  ...
                  <select value={localFilters.maxAge}
...
function ProfileCard({ profile }: { profile: any }) {
  return (
    // ✅ CORREÇÃO: Link do next/link em vez de <a>
    <Link
      ...
      <div className="absolute bottom-0 left-0 right-0 p-3">
        ...
        {profile.distance_km !== undefined && (
          <p className="flex items-center gap-1 text-white/50 text-xs mt-0.5">
            <MapPin size={9} />
            {profile.distance_km < 1 ? 'menos de 1 km' : `${profile.distance_km} km`}
          </p>
        )}
      </div>
    </Link>
